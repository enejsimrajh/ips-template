...
public class UserService : IUserService
{
    ...
    public async Task<(IList<TEIdentityUser> Users, int TotalCount)> GetUsersAsync(UserFilter filter)
    {
        ...
        {
            filteredUsers = filteredUsers.Where(u => u.PhoneNumber.Contains(filter.PhoneNumber, StringComparison.OrdinalIgnoreCase));
        }
        if (!string.IsNullOrEmpty(filter.Name))
        {
            filteredUsers = filteredUsers.Where(u => u.Name.Contains(filter.Name, StringComparison.OrdinalIgnoreCase));
        }
        if (!string.IsNullOrEmpty(filter.Surname))
        {
            filteredUsers = filteredUsers.Where(u => u.Surname.Contains(filter.Surname, StringComparison.OrdinalIgnoreCase));
        }
